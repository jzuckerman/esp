...
#define SLD_SVHN_DENOISE 0x091
#define DEV_NAME "sld,svhn_denoise_hls4ml"

#define INT_BITS 22
#define fl2fx(A) float_to_fixed32(A, INT_BITS)

/* <<--params-->> */
const int32_t nbursts = ;
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, SLD_SVHN_DENOISE, DEV_NAME);
	if (ndev == 0) {
		printf("svhn_denoise not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
		iowrite32(dev, SVHN_DENOISE_NBURSTS_REG, nbursts);
