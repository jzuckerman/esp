...
#define SLD_MNIST_AUTOENC 0x092
#define DEV_NAME "sld,mnist_autoenc_hls4ml"
...
static unsigned in_words = 784;
static unsigned out_words = 784;
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, SLD_MNIST_AUTOENC, DEV_NAME);
	if (ndev == 0) {
		printf("mnist_autoenc not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
		iowrite32(dev, MNIST_AUTOENC_NBURSTS_REG, nbursts);
