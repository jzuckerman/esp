	...
	*loop_cnt = malloc(sizeof(unsigned) * *nthreads); 
	...
	for (int t = 0; t < *nthreads; t++){
		...
		for (int d = 0; d < thread_info[t]->ndev; d++){
			...
			fscanf(f, "%s %s", thread_info[t]->workload_size, coh_choice);
			
            unsigned s = SMALL;
            if (!strcmp(thread_info[t]->workload_size, "S")) {
                s = SMALL;
            } else if (!strcmp(thread_info[t]->workload_size, "M")) {
                s = MEDIUM;
            }else if (!strcmp(thread_info[t]->workload_size, "L")) {
                s = LARGE;
            }            
            ...
            switch(devid){
                ...
                case MRIQ : 
                    ...
                    if (s == SMALL) {
                        ...
                        mriq_cfg_000[0].batch_size_x = 128;
                        mriq_cfg_000[0].batch_size_k = 1024;
                        mriq_cfg_000[0].num_batch_x = 96;
                        mriq_cfg_000[0].num_batch_k = 96;
                }
			...
			if (thread_info[t]->p2p){
                ...
                if (!strcmp(accnames[devid], "fft"))
                    ((*cfg)[t][d].esp_desc)->coherence = ACC_COH_LLC;
                else if (!strcmp(accnames[devid], "sort"))
                    ((*cfg)[t][d].esp_desc)->coherence = ACC_COH_NONE;
                else if (!strcmp(accnames[devid], "spmv"))
                    ((*cfg)[t][d].esp_desc)->coherence = ACC_COH_RECALL;
                else if (!strcmp(accnames[devid], "visionchip"))
                    ((*cfg)[t][d].esp_desc)->coherence = ACC_COH_RECALL;
                else 
                    printf("Invalid accelerator name\n");
            }
...
static void load_spmv(uint32_t* buffer, accelerator_thread_info_t *thread_info)
{
    ...
    if (!strcmp(thread_info->workload_size, "S")) {
        size = 5794;
        buf = spmv_buf_s;
    } else if(!strcmp(thread_info->workload_size, "M")) {
        size = 39224;
        buf = spmv_buf_m;
    } else if(!strcmp(thread_info->workload_size, "L")) {
        size = 1141362;
        buf = spmv_buf_l;
    } else {
        return;
...
static void read_spmv(int32_t* buffer, char *size)
{
	FILE *fp = NULL;
	char str_tmp[4];
	int i;
...
int main (int argc, char** argv)
{
    ...
    read_spmv(spmv_buf_s, "S");
    read_spmv(spmv_buf_m, "M");
    read_spmv(spmv_buf_l, "L");
