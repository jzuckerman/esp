...
#define FFT2 6
#define SORT1 7
#define NIGHTVISION1 8
#define SORT2 9
#define SPMV2 10
#define NIGHTVISION2 11
		...
		for (int d = 0; d < thread_info[t]->ndev; d++){
            ...
            switch(devid){
                ...
                case SORT2 :
                    ...
                    sort_cfg_000[0].esp.devid = devid;
                    if (s == EXTRA_SMALL) {
                ...
                case NIGHTVISION2 :
                    ...
                    nightvision_cfg_000[0].esp.devid = devid;
                    if (s == EXTRA_SMALL) {
                ...
                case SPMV2 : 
                    ...
                    spmv_cfg_000[0].esp.devid = devid;
                    if (s == EXTRA_SMALL) {
...
int main (int argc, char** argv)
{
	...
	if (argc < 4){
		printf("Usage: ./parallel_acc_coh.exe file coherence alloc [epsilon] [alpha]\n");
		return -1;
	}
	//command line args
	FILE* f;
	...
	int test_no = 0;
	printf("test_no %d\n", test_no);
	const char* out_name_d = "parallel_acc_coh_devices.csv";
	const char* out_name_t = "parallel_acc_coh_threads.csv";
	const char* out_name_p = "parallel_acc_coh_phases.csv";
